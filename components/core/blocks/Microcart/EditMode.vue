...
export default {
  ...
  methods: {
    openEditMode() {
      this.$store
        .dispatch("themeCart/configureProduct", { product: this.product })
        .then(() => {
          const selectedOptions = Object.assign({}, this.getSelectedFilters);
          this.$store.dispatch("themeCart/openEditMode", {
    ...
    updateVariant() {
      ...
      this.$store.dispatch("cart/configureItem", {
        product: this.product,
        configuration
      });
    },
